...
> **项目状态：** Phase 1-3 已完成——券商连接、行情数据、风控和交易日志已可对接 Alpaca 模拟盘账户；策略引擎 + 哨兵循环（YAML 策略、规则求值、版本管理、定时监控、硬规则自动执行）现已运行；LLM agent 核心（多 provider 聊天客户端、工具调用循环、`tradewind chat` REPL，以及在软规则触发进入待确认队列前先做分析标注的 ReviewAgent）也已就位。下一步是 Web UI + 记忆系统，详见[路线图](#路线图)。**默认仅模拟盘（paper trading）。**
