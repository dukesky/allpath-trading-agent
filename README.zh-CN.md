...
> **项目状态：** Phase 1（执行地基）已完成——券商连接、行情数据、风控和交易日志已可对接 Alpaca 模拟盘账户。LLM agent 核心、策略引擎、记忆系统与 Web UI 正在开发中，详见[路线图](#路线图)。**默认仅模拟盘（paper trading）。**
...
- **三个运行循环**
  ...
  2. **哨兵循环**——盘中每 2 小时（可配置）：确定性代码将价格与策略规则逐条比对，零 LLM 成本。触发后，*硬规则*（如止损）立即执行、全程无 LLM；*软规则*（如逢低加仓）先唤醒 agent 研究当下状况。
...
| 2 | **策略引擎 + 哨兵循环**——YAML 策略文档、受限表达式规则求值器、版本管理、定时监控、硬规则自动执行 | 🔜 下一步 |
| 3 | **Agent 核心**——多 provider LLM 层（Claude / OpenAI / OpenRouter）、工具循环、上下文组装 | 计划中 |
