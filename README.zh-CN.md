...
"不留存"具体是指：图片不会写进数据库、搜索索引、日志，或任何本程序保留
的文件——一轮对话留下的唯一痕迹就是那行 `[image: 名称, 大小]` 占位文字。
网页上传这条路上，HTTP 层在解析请求时仍可能把较大的请求暂存到一个**没有
文件名**的临时文件里，这发生在本程序自己的代码运行之前；该文件不出现在
文件系统里，只属于这一次请求，请求结束即释放。超过四张图片额度的请求会
在解析之前就被直接拒绝。
